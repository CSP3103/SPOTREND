...
async def analizar_cancion_completo_html(
    request: Request,
    cancion_id: str,
    session: Session = Depends(get_session)
):
    """Analizar canción (HTML)"""
    try:
        ...
        if not benchmarks:
            return templates.TemplateResponse("analisis/sin-benchmarks.html", {
                "request": request,
                "cancion": cancion
            })
        ...
        return templates.TemplateResponse("analisis/detalle.html", {
            "request": request,
            "analisis": {
                "cancion": cancion,
                "resumen": {
                    ...
                    "afinidad_promedio": round(sum(r["afinidad"] for r in resultados) / len(resultados), 1) if resultados else 0
                },
        ...
        session.rollback()
        logger.error(f"Error en análisis HTML: {e}")
        return templates.TemplateResponse("error.html", {
            "request": request,
            "error": f"Error en análisis: {str(e)[:100]}"
        })

@router.get("/tendencias", response_class=HTMLResponse)
async def analizar_tendencias_html(
    request: Request,
    dias: int = 7,
    session: Session = Depends(get_session)
):
    """Tendencias de análisis (HTML)"""
    try:
        ...
        if not analisis_recientes:
            return templates.TemplateResponse("analisis/tendencias.html", {
                "request": request,
                "datos": {
        ...
        return templates.TemplateResponse("analisis/tendencias.html", {
            ...
            "datos": {
                ...
                "tendencias": lista_tendencias[:10]
            }
        })

    except Exception as e:
        logger.error(f"Error analizando tendencias HTML: {e}")
        return templates.TemplateResponse("error.html", {
            "request": request,
            "error": f"Error analizando tendencias: {str(e)}"
        })
...
async def analizar_cancion_completo_v2(
    ...
    """API: Analizar canción (JSON) - ORIGINAL"""
    try:
        await asyncio.sleep(0.01)
        cancion = session.get(Cancion, cancion_id)
        if not cancion:
...
@router.get("/api/tendencias")
async def analizar_tendencias_v2(
    session: Session = Depends(get_session)
):
    """API: Tendencias (JSON) - ORIGINAL"""
    try:
